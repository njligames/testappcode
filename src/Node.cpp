...
namespace NJLIC {
    Node::Node()
        : m_Name("NODE"), mScale(new glm::vec3(1.0f, 1.0f, 1.0f)),
          ...
          m_Geometry(NULL), m_GeometryIndex(-1), m_HideGeometry(false),
          m_Opacity(1.0f), mNormalMatrix(new glm::mat3x3(1)),
          mColorbase(new glm::vec4(1, 1, 1, 1)), m_TransformDirty(true),
          m_NormalMatrixDirty(true), m_OpacityDirty(true), m_HiddenDirty(true),
    ...
    Geometry *const Node::getGeometry() const { return m_Geometry; }
    ...
    float Node::getOpacity() const { return m_Opacity; }
    ...
    const glm::mat3x3 &Node::getNormalMatrix() const { return *mNormalMatrix; }
    ...
    const glm::vec4 &Node::getColorBase() const { return *mColorbase; }

    Node *Node::getParentNode() { return m_ParentNode; }

    const Node *Node::getParentNode() const { return m_ParentNode; }

    bool Node::hasParentNode() const { return (NULL != m_ParentNode); }
    ...
    void Node::removeParentNode() { m_ParentNode = NULL; }
    ...
    Node *Node::findChildNode(const std::string &name) {
        if (getName() == name) {
            ...
            for (std::vector<Node *>::const_iterator iter =
                     m_ChildrenNodes.begin();
                 iter != m_ChildrenNodes.end(); ++iter) {
    ...
    unsigned long Node::getChildNodeIndex(Node *object) const {
        std::vector<Node *>::const_iterator iter =
            std::find(m_ChildrenNodes.begin(), m_ChildrenNodes.end(), object);
    ...
    bool Node::hasChildNode(Node *object) const {
        ...
        for (iter = m_ChildrenNodes.begin(); iter != m_ChildrenNodes.end();
             ++iter) {
    ...
    const glm::mat4x4 &Node::getTransform() const { return *mTransform; }
    ...
    unsigned long Node::getGeometryIndex() const { return m_GeometryIndex; }
