...
bool Ishne::init(const std::string &fileName) {
    ...
    if (nullptr != data) {
        ...
        memcpy(&mData->Sampling_Rate, current_ptr,
               sizeof(char) * 2); // 274 bytes
        current_ptr += 2;
        currentByteCount += 2;

        memcpy(&mData->Proprietary, current_ptr,
               sizeof(char) * 80); // 354 bytes
        current_ptr += 80;
        currentByteCount += 80;
        ...
        if (mData->Var_length_block_size > 0) {
            ...
            memcpy(varblock, current_ptr,
                   sizeof(char) * mData->Var_length_block_size); // 522 bytes
        ...
        for (int i = 0; i < mData->Sample_Size_ECG; i++) {
            ...
            for (int j = 0; j < mData->nLeads && (currentByteCount < mFileSize);
                 j++) {
                short data;
            ...
            if (currentByteCount >= mFileSize) {
                ...
                long specification_file_size =
                    8 + 2 + 512 + mData->Var_length_block_size +
                    (mData->Sample_Size_ECG * mData->nLeads);

                std::string err_str = "The file `";
                err_str += fileName;
                err_str += "` was corrupt. According to the file "
                           "specifications the file should be ";
                ...
                err_str += units;
                err_str += "bytes less then what is expected, according to "
