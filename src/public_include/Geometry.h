...
//
        ...
        TexturedColoredVertex()
            : vertex(0, 0, 0), color(1, 1, 1, 1), texture(0, 0),
              normal(1, 1, 1), tangent(0, 0, 0), bitangent(0, 0, 0)
        //        , opacity(1.0f)
        //        , hidden(0.0f)
        {}
        TexturedColoredVertex(
            const glm::vec3 vertex, const glm::vec4 color,
            const glm::vec2 texture, const glm::vec3 normal,
            const glm::vec3 tangent, const glm::vec3 bitangent
            //                              const GLfptype opacity,
            //                              const GLfptype hidden
            )
            : vertex(vertex), color(color), texture(texture), normal(normal),
        ...
        operator std::string() const {
            ...
            //            %f}, {%f, %f, %f}, %f, %f}", sprintf(buffer, "{{%f,
            //            %f, %f}, {%f, %f}, {%f, %f, %f}, %f, %f}",
            sprintf(buffer, "{{%f, %f, %f}, {%f, %f}, {%f, %f, %f}}", vertex.x,
                    vertex.y, vertex.z, texture.x, texture.y,
    ...
    class Geometry {
      ...
      public:
        ...
        virtual void loadDiffuseMatrial(Shader *shader,
                                        const unsigned char *diffuseFileData,
                                        int width, int height,
                                        int channels_in_file);

        virtual bool loadDiffuseMatrial(Shader *shader,
      ...
      protected:
        ...
        void setNormalMatrixTransform(const unsigned long index,
                                      const glm::mat4x4 &transform);
